...
- `--plugin-ui-path` 可让 UI `package.json` 使用本地 `vesty-plugin-ui` file dependency；React/Vue/Svelte adapter 通过同一个包的子路径导出，无需额外 sibling package。
...
- 可选生成/检查 npm package dry-run report: `vesty npm-pack --out <path>` 会运行 `npm pack --workspaces --dry-run --json`，写入规范 JSON，并立即复用 release gate 的包边界校验；`vesty npm-pack --check --out <path>` 只复验已有 report。`release-check --npm-pack-report <path>` 要求唯一发布包 `vesty-plugin-ui` 存在，且 packed files 只包含 `dist/**` 和 `package.json`；React/Vue/Svelte adapter 作为 `dist/react.*`、`dist/vue.*` 和 `dist/svelte.*` 子路径入口随包发布。显式 report 文件不能是 symlink。普通检查缺失时为 `skipped`；`--require-release-artifacts` 会要求该 evidence 存在并有效，避免 npm package 边界退化后仍宣称 release-ready。
