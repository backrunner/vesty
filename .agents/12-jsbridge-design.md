...
- `@vesty/react`: hooks 和 store adapter。
- `@vesty/vue`: composables。
- `@vesty/svelte`: stores。

`@vesty/plugin-ui` 是底层稳定 API；`@vesty/react`、`@vesty/vue`、`@vesty/svelte` 是薄适配，只包装 bridge、snapshot store 和参数编辑 helper，不引入 Tauri command 风格 API。Rust IPC 生成的协议类型会随 `@vesty/plugin-ui` 发布，并通过 `@vesty/plugin-ui/protocol` 子路径暴露给 UI 项目使用。
...
- `@vesty/react`: `VestyBridgeProvider`、`useVestyBridge()`、`useVestySnapshotStore()`、`useVestySnapshot()`、`useVestyParamEdit()`。
- `@vesty/vue`: `useVestySnapshot()`、`useVestyParamEdit()`。
- `@vesty/svelte`: `vestySnapshotStore()`、`vestyParamEdit()`。
