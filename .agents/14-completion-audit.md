...
> 前端 SDK 现在只发布一个 npm package：`vesty-plugin-ui`，并通过 `/protocol`、`/react`、`/vue`、`/svelte` 提供子路径导出。下方较早日期的四包验证记录仅作为历史证据保留，不代表当前发布结构。
...
| JS package `vesty-plugin-ui` | Done | TypeScript build/typecheck/test；protocol export snapshot check；React/Vue/Svelte 子路径 adapters exist；JS package metadata/exports/files 已补齐；`vesty npm-pack --out` 会生成并复验单个 npm package dry-run report；`release-check --npm-pack-report` 可验证 tarball 发布边界 | npm publish execution 和 registry credentials |
...
The latest full local verification passed after the bundle-label and dependency evidence refreshes: `cargo test --workspace -j1` reported 594 passed, workspace Clippy reported no warnings with `-D warnings`, and `npm test` passed across `vesty-plugin-ui`, `vesty-plugin-ui/react`, `vesty-plugin-ui/svelte` and `vesty-plugin-ui/vue`. This is local implementation evidence only; it still does not prove multi-DAW, cross-platform WebView, validator matrix, signing or notarization readiness.
...
rtk npm install --workspace vesty-plugin-ui/vue vue@latest --save-dev
rtk cargo test -p vesty-cli dependency_baseline -- --nocapture
