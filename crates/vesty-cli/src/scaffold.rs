...
"#
    ...
    if is_instrument {
        fs::write(
            ...
            include_str!("templates/instrument_tests.rs.txt")
                .replace("__PLUGIN__", &plugin_type)
                .replace("__KERNEL__", &kernel_type),
        )?;
    }
    if with_ui {
        write_ui_template(
...
pub(super) fn cargo_toml(
    ...
    let source = match vesty_path {
        Some(path) => format!(r#"path = "{}""#, toml_escape(path.as_str())),
        None => format!(r#"version = "={}""#, env!("CARGO_PKG_VERSION")),
    };
    let feature = if with_ui {
        "vst3-wry-ui"
    } else {
        "vst3-bindings"
    };
    let vesty_dependency =
        format!(r#"vesty = {{ {source}, default-features = false, features = ["{feature}"] }}"#);
    format!(
        r#"[package]
name = "__CRATE_NAME__"
version = "0.1.0"
...
Run all commands below from the project root. If generated with local source overrides,
keep that Vesty checkout available: Cargo and npm dependencies refer to its absolute path.
...
vesty package --config vesty.toml --binary target/release/lib{crate_name}.dylib
...
The package command above uses the macOS binary. On Windows use
`target/release/{crate_name}.dll`; on Linux use `target/release/lib{crate_name}.so`.
The target platform defaults to the current host. Static validation does not replace
Steinberg validator or DAW testing.

Continue with the [plugin tutorial](https://vesty.pwp.sh/docs/guides/complete-plugin)
and [MIDI guide](https://vesty.pwp.sh/docs/guides/midi). The instrument starter is
monophonic with last-note priority; polyphony, sustain and expression require additional DSP.
"#,
...
pub struct {kernel_type} {{
    ...
    phase: f32,
    frequency: f32,
    active_note: Option<(u16, u8, i32)>,
    volume: ParamHandle,
}}

impl AudioKernel for {kernel_type} {{
    fn prepare(&mut self, context: PrepareContext) {{
        self.sample_rate = context.sample_rate as f32;
    }}
    ...
    fn process(&mut self, context: &mut ProcessContext<'_>) -> ProcessResult {{
        let mut volume = context.param_normalized(self.volume).unwrap_or(0.2) as f32;
        let (audio, events) = context.audio_mut_and_events();
        audio.clear_outputs();
        let frames = audio.frames();
        let mut cursor = 0;
        // Zero-frame contexts still deliver events during dense host blocks.
        for event in events {{
            let offset = (event.sample_offset() as usize).min(frames);
            self.render(audio, cursor, offset, volume);
            cursor = offset;
            match *event {{
                Event::NoteOn {{ channel, key, velocity, note_id, .. }} if velocity > 0.0 => {{
                    self.active_note = Some((channel, key, note_id));
                    self.frequency = 440.0 * 2.0_f32.powf((key as f32 - 69.0) / 12.0);
                }}
                Event::NoteOff {{ channel, key, note_id, .. }}
                | Event::NoteOn {{ channel, key, note_id, velocity: 0.0, .. }} => {{
                    if self.active_note.is_some_and(|(active_channel, active_key, active_id)| {{
                        active_channel == channel
                            && if active_id >= 0 && note_id >= 0 {{
                                active_id == note_id
                            }} else {{
                                active_key == key
                            }}
                ...
                Event::Param {{ handle, normalized, .. }} if handle == self.volume => {{
                    volume = normalized as f32;
                }}
                _ => {{}}
            }}
        }}
        self.render(audio, cursor, frames, volume);
        ProcessResult::Continue
    }}
}}

impl {kernel_type} {{
    fn render(&mut self, audio: &mut AudioBuffers<'_>, start: usize, end: usize, volume: f32) {{
