...
Vesty 仍处于 alpha 阶段，当前最可靠的方式是基于 Vesty 的本地源码目录进行开发：
...
在 Vesty 源码目录中，可以使用 `cargo run -p vesty-cli -- <command>` 执行后文的命令；在同级的插件工程中，则使用 `cargo run --manifest-path ../vesty/Cargo.toml -p vesty-cli -- <command>`。如果已经安装 CLI，直接运行 `vesty <command>` 即可。
...
如果目录结构不同，请相应调整路径。当 Vesty 发布了适合产品使用的版本后，应固定一个明确的兼容版本，不要让正式构建持续跟随变化中的 Git 分支。
