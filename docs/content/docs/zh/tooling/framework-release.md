...
- `crates-io` 保护 `publish-crates` job。为每个 Vesty crate 添加 GitHub trusted publisher：repository owner 填 `orchiliao`，repository name 填 `vesty`，workflow filename 填 `release.yml`，environment 填 `crates-io`。
- `npm` 保护 `publish-npm` job。为四个 `@vesty/*` 包添加 GitHub Actions trusted publisher：repository owner 填 `orchiliao`，repository name 填 `vesty`，workflow filename 填 `release.yml`，environment 填 `npm`。

两个环境都应启用 reviewer approval。workflow 会申请 GitHub OIDC identity token，并换取短期 registry 凭据，不读取 crates.io 或 npm 发布 secret。

配置 publisher 前，GitHub 仓库必须已经存在于 `orchiliao/vesty`。如果最终 repository owner 发生变化，应同步修改文档，并在 17 条 publisher 记录中使用准确 owner。
